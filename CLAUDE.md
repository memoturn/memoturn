...
- **`apps/worker`** — BullMQ consumers. `processors/ingest.ts` (merge → ClickHouse + online evals), plus daily maintenance crons (retention sweep `0 3 * * *`, scheduled blob exports `0 4 * * *`) and a health/metrics HTTP endpoint (`WORKER_PORT`, default 3002). `mappers.ts` converts ingest events → ClickHouse rows (has the only worker tests).
