...
bun run infra:up / infra:down / infra:logs / infra:status   # docker compose for PG/Doris/Redis/MinIO (TELEMETRY_ENGINE=postgres skips the Doris containers)
...
- **`apps/api`** — Hono + `@hono/zod-openapi`. `src/app.ts` is the entire route surface; **handlers are thin** — they call into `@memoturn/server`. Same app is served by `server.bun.ts` (primary) and `server.node.ts`. Auth resolution in `src/middleware/auth.ts`. Global hardening middleware (applied at the top of `app.ts`): `secureHeaders` (X-Frame-Options/DENY, no-referrer, etc.), CORS scoped to `AUTH_TRUSTED_ORIGINS`, and request body-size limits (1 MB default, 12 MB for `/v1/ingest`, `/v1/otel/*`, and `/v1/media`). Observability (`src/metrics.ts`): a first middleware times every request and emits a structured JSON log line; in-process metrics (request counts, status classes, per-route latency percentiles bucketed by route pattern, in-flight gauge) are exposed at `GET /metrics` — token-gated (404 unless `API_METRICS_TOKEN` is set, then `Authorization: Bearer <token>`), as JSON by default or Prometheus text when the scraper sends `Accept: text/plain` / `?format=prometheus`. `GET /health` is a public liveness probe; `GET /ready` (from `packages/server/src/readiness.ts`, also served by the worker) pings Postgres/Redis/telemetry/blob and is what orchestrator readiness checks should target. Every response carries an `x-request-id` (inbound honoured) that is stamped on log lines, error bodies (`app.onError` → JSON `{error, requestId}`; blob/queue outage → 503 + `Retry-After`), and the ingest job.
- **`apps/worker`** — BullMQ consumers. `processors/ingest.ts` (merge → telemetry store + online evals), `processors/experiment.ts` (server-executed dataset runs), `processors/eval-backfill.ts` (run an evaluator over already-ingested traces; own Worker on the `eval` queue at `EVAL_BACKFILL_CONCURRENCY`, default 1), plus daily maintenance crons (retention sweep `0 3 * * *`, scheduled blob exports `0 4 * * *`, embedding-projection reduction `0 5 * * *`), an hourly mutable-state prune (`17 * * * *` — bounds the Postgres `*State` working set per ADR-0001), a per-minute alert-evaluation cron (`* * * * *` — stateful alert rules + cost budgets + thread-scope conversation evaluators past their cooldown), a daily demo-sandbox prune (`30 3 * * *` — public demo only, registered when `DEMO_MODE` is on), and a health/metrics HTTP endpoint (`WORKER_PORT`, default 3002). `mappers.ts` converts ingest events → engine-neutral telemetry rows, including the computed `latency_ms` (has the only worker tests). Key hardening details: jobs that exhaust retries land in a **dead-letter queue** (`ingest-dlq`) for inspection/replay; each telemetry table (`traces`, `observations`, `scores`) is inserted **independently** so one table failure doesn't discard the others; large input/output payloads (> 256 KB) are **offloaded to blob** with a marker reference before insert; all log output is **structured JSON** (`logJson`); counters (`ingest_events_total`, `ingest_errors_total`, `ingest_rows_total`, `evaluator_runs_total`) plus `telemetry_insert` latency and `dlqDepth` are exposed in the `/metrics` JSON; retention and export crons are guarded by a **Redis lock** (`withLock`) to prevent concurrent runs across multiple worker replicas. Per-event token counts are clamped to `MAX_EVENT_TOKENS` (10 M) in `packages/core/src/models.ts` to prevent runaway cost inflation.
...
- **`apps/web`** — public marketing site (memoturn.com). TanStack Start (React SSR) + Vite + Tailwind 4, deployed as a Cloudflare Worker (`wrangler.jsonc`; staging via `CF_ENV=staging`). Dev port **3003**. The whole landing page is `src/routes/index.tsx`; shared design system from `@memoturn/ui`.
- **`apps/docs`** — public docs site (docs.memoturn.com). Astro + Starlight, static-assets Cloudflare Worker. Dev port **4321**. Content in `src/content/docs/` mirrors the hand-written `docs/*.md` (which stay the source of truth — update both when product facts change).
- **`apps/mcp`** — stdio MCP server exposing prompts/datasets/review queues as tools for agent IDEs. Scoped to one project via a `MEMOTURN_PUBLIC_KEY`/`MEMOTURN_SECRET_KEY` pair (resolved through `@memoturn/server`). The tool registry lives in **`packages/server/src/mcp-tools.ts`** (plain JSON Schema, no zod coupling) and is shared with the remote HTTP endpoint; `index.ts` wires the low-level `Server` via `serveStdio`. **All logging goes to stderr** — stdout is the JSON-RPC channel. Both surfaces run on `@modelcontextprotocol/server` v2 and speak the **stateless 2026-07-28 protocol** (SEP-2575: no initialize handshake) while still serving pre-2026 clients (`legacy: "stateless"` over HTTP, era-pinned over stdio). The same registry is also served over **Streamable HTTP** by the API at `/v1/mcp/:projectId` (per-project resource; own API-key Basic auth + per-tool RBAC in `apps/api/src/mcp.ts`, via `createMcpHandler`'s fetch face) — don't add a `requireAuth` guard there, its method-based scope gate can't distinguish read tools from writes (every call is a POST).
...
- **`packages/telemetry`** — the **analytical-store seam**: the `TelemetryStore` interface (`src/store.ts`, ~20 domain methods typed against `@memoturn/contracts`) and its two engine implementations — **Apache Doris** (`src/doris/`, mysql2 against the FE; the default) and **Postgres** (`src/postgres/`, node-pg into the `telemetry` schema; selected by `TELEMETRY_ENGINE=postgres`, needs pgvector — ADR-0002). All telemetry SQL lives here — packages/server and the worker call store methods, never hand-written engine SQL. Tables share one LWW identity (`TELEMETRY_PRIMARY_KEYS` in `src/types.ts`) with sequence column `event_ts` (last-writer-wins: re-inserting an id with a newer-or-equal event_ts overwrites; that's what makes ingest retries and score corrections idempotent) — Doris via UNIQUE KEY merge-on-write, Postgres via `ON CONFLICT … WHERE excluded.event_ts >= t.event_ts` upserts. `src/migrate.ts` dispatches on the engine (`infra/doris/*.sql` vs `infra/postgres-telemetry/*.sql`, each with a `schema_migrations` ledger). `conformance.test.ts` is the behavioral contract — CI runs it against **both** engines, so any store change must land in both dialects. `scanRows` is the engine-to-engine bulk-copy seam (ADR-0004). Gotcha: never insert ARRAY columns via `CAST('[…]' AS ARRAY<STRING>)` — Doris's string→array parser corrupts values with quotes/commas; use array constructors with per-element placeholders (see `doris/serialize.ts`). Writes default to multi-row INSERT; setting `TELEMETRY_STREAM_LOAD=true` switches `insertRows` to the HTTP **Stream Load** path (`doris/streamload.ts`) — higher throughput, native `ARRAY<FLOAT>` JSON, `event_ts` as the sequence column so LWW still holds. In-network deploys hit the FE (8030, which redirects to a BE); a host-run worker points `DORIS_STREAM_LOAD_PORT` at a BE (8040) to load it directly.
...
- **Engine policy — two engines, behind the seam** (ADR-0002): Apache Doris (default, the scale engine) and Postgres (`TELEMETRY_ENGINE=postgres`, the small-install tier — telemetry in the OLTP Postgres's `telemetry` schema, pgvector, no Doris containers). All engine SQL stays inside `packages/telemetry` (`src/doris/` + `src/postgres/`); **every store change lands in BOTH dialects** and `conformance.test.ts` (which CI runs against both engines) is the contract. Engine-to-engine moves use the `scanRows`→`insertRows` copy path (ADR-0004); blob replay is the fallback. Don't scatter engine SQL into packages/server or the worker.
- **Design for deployment-profile portability** (see `docs/adr/0002` + `0003`): new designs must work across container and future serverless/edge profiles. Concretely — new infra dependencies go behind a seam (like `TelemetryStore`) or must have a managed/serverless equivalent; don't assume a long-lived process or in-process state (caches, counters) without a per-invocation-safe fallback; queue messages carry blob pointers, never payloads; keep the API Hono-portable (runtime-specific APIs live in entrypoints); don't deepen Redis-lock coupling in crons.
