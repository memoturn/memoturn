...
- Docker (Postgres, Apache Doris, Redis/Valkey, MinIO) — give Docker ~4 GB of memory; the Doris FE + BE want it. Don't want to run Doris? Set `TELEMETRY_ENGINE=postgres` in `.env` for the lightweight [Postgres telemetry tier](./deployment.md#telemetry-engine-doris-or-postgres) — every step below stays the same, minus the Doris containers.
