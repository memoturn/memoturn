...
| OLAP | Apache Doris **or** Postgres (`TELEMETRY_ENGINE`) | `traces`, `observations`, `scores` + RAG tables; dashboard metrics are aggregated on the fly from `observations`. Doris (default): `UNIQUE KEY` merge-on-write tables, the scale engine. Postgres: the small-install tier ([ADR-0002](./adr/0002-postgres-telemetry-tier.md)) — same tables in the OLTP Postgres's `telemetry` schema, LWW upserts + pgvector, **no Doris containers** |
...
([ADR-0004](./adr/0004-telemetry-graduation-path.md)).
