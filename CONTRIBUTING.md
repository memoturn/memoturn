...
- **Docker** (for Postgres, Apache Doris, Redis/Valkey, MinIO — Doris FE+BE want ~4 GB of Docker memory)
...
```bash
cp .env.example .env
bun run setup   # install + infra up + wait-for-healthy + migrate + telemetry DDL + seed
...
apps/worker    Bun + BullMQ — ingest -> Doris, online evaluators
...
packages/db    Prisma (Postgres) + blob/queue clients
...
the datastore env (`DATABASE_URL`, `DORIS_HOST`, `REDIS_URL`, `BLOB_ENDPOINT`) is set,
...
- **Data flow**: SDK/OTel → `POST /v1/ingest` → blob (raw event log) → BullMQ → worker → Doris. Relational metadata is in Postgres (Prisma); high-volume telemetry is in Apache Doris, behind the `TelemetryStore` interface in `packages/telemetry`.
