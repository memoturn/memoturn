...
export async function applyRetention(projectId: string, days: number): Promise<RetentionResult> {
  ...
  const before = await ch
    .query({ query: `SELECT count() AS c FROM traces FINAL WHERE project_id = {p:String} AND timestamp < ${cutoff}`, query_params: { p: projectId }, format: "JSONEachRow" })
    .then((r) => r.json<{ c: string }>())
    .then((rows) => Number(rows[0]?.c ?? 0));

  await ch.command({ query: `DELETE FROM traces WHERE project_id = {p:String} AND timestamp < ${cutoff}`, query_params: { p: projectId } });
  await ch.command({ query: `DELETE FROM observations WHERE project_id = {p:String} AND start_time < ${cutoff}`, query_params: { p: projectId } });
  await ch.command({ query: `DELETE FROM scores WHERE project_id = {p:String} AND timestamp < ${cutoff}`, query_params: { p: projectId } });
