...
export class DorisTelemetryStore implements TelemetryStore {
  ...
  private observationListWhere(projectId: string, filters: ObservationFilters): { where: string; params: unknown[] } {
    ...
    if (search) {
      ...
      conds.push(
        `(LOWER(o.name) LIKE CONCAT('%', LOWER(?), '%')
          OR LOWER(o.input) LIKE CONCAT('%', LOWER(?), '%')
          OR LOWER(o.output) LIKE CONCAT('%', LOWER(?), '%'))`,
      );
  ...
  async listObservations(projectId: string, filters: ObservationFilters = {}): Promise<ObservationSummary[]> {
    ...
    const rows = await this.query<Omit<ObservationSummary, "trace_name">>(
      ...
      SELECT
        ...
        DATE_FORMAT(o.start_time, ${ISO_FMT}) AS start_time,
        IF(o.end_time IS NULL, NULL, DATE_FORMAT(o.end_time, ${ISO_FMT})) AS end_time,
  ...
  async observationFacets(
    ...
    // so a selected value still shows the alternatives you could switch to.
  ...
  async scoreStats(projectId: string, name: string, days: number): Promise<ScoreStatsRow> {
    const [row] = await this.query<Record<string, unknown>>(
      ...
      SELECT
        ...
        PERCENTILE_APPROX(\`value\`, 0.5) AS p50_v,
        PERCENTILE_APPROX(\`value\`, 0.95) AS p95_v
      FROM scores
  ...
  async scoreTimeline(
    ...
    const rows = await this.query<{ date: string; cnt: unknown; mean_v: unknown }>(
      `
      SELECT DATE_FORMAT(\`timestamp\`, '%Y-%m-%d') AS date, COUNT(*) AS cnt, AVG(\`value\`) AS mean_v
      FROM scores
  ...
  async retrievalAnalytics(
    ...
    // RETRIEVER observation, so grouping by observation_id is the natural unit.
    const rollupSql = `
    ...
    const [summaryRows, histRows, weakRows, docRows] = await Promise.all([
      ...
      // 10 buckets over [0,1]. A perfect 1.0 would land in bucket 10, so it is folded back
      // into the top bucket rather than creating an 11th.
      ...
      this.query<{
        ...
        `SELECT r.trace_id AS trace_id, r.observation_id AS observation_id,
                ...
                DATE_FORMAT(r.event_ts, ${ISO_FMT}) AS timestamp,
