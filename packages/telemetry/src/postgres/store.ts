...
import { keysetAfter, parseTags, parseVector, toDorisDateTime } from "../serialize-shared.js";
 ...
 * telemetry tier). A method-for-method port of doris/store.ts: same two-step list
 * queries, same facet-excluding composition, same JS-computed cutoffs and Number()
 * normalization at the boundary (node-pg surfaces int8/numeric as strings). Dialect
 * differences are mechanical: to_char for ISO formats, FILTER for conditional counts,
 * percentile_cont for percentiles, LATERAL unnest for tag explosion, pgvector `<=>`
 * for cosine distance. Bare table names resolve via the pool's search_path.
 */

const ISO_FMT = `'YYYY-MM-DD"T"HH24:MI:SS"Z"'`;
// Millisecond-preserving variant for write-shaped row reads: event_ts is the LWW
// sequence value, so read-merge bases must not truncate it. .MS is 3 digits (Doris %f
// emits 6); both parse to identical epoch-millis through JS Date.
const ISO_MS_FMT = `'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"'`;
...
/** UTC-midnight-anchored cutoff N days ago (calendar-aligned, keeps the first bucket complete). */
...
/** ISO timestamp for "now minus N minutes" as a timestamp literal (short-window metrics). */
...
export class PostgresTelemetryStore implements TelemetryStore {
  ...
  private traceListWhere(projectId: string, filters: TraceFilters): { where: string; params: unknown[] } {
    ...
    if (traceIds && traceIds.length > 0) {
      // Restrict to an explicit id set (pgQuery expands a single `?` bound to an array).
      conds.push("t.id IN (?)");
      ...
      conds.push(
        `(LOWER(t.name) LIKE '%' || LOWER(?) || '%'
          OR t.id IN (
            ...
            WHERE project_id = ?
              AND (LOWER(input) LIKE '%' || LOWER(?) || '%' OR LOWER(output) LIKE '%' || LOWER(?) || '%')
          ))`,
      );
      params.push(search, projectId, search, search);
    }
    if (tag) {
      conds.push("? = ANY(t.tags)");
      params.push(tag);
    }
    if (promptId) {
  ...
  async traceHistogram(
    ...
    const fmt = interval === "hour" ? `'YYYY-MM-DD"T"HH24":00"'` : "'YYYY-MM-DD'";
    const { where, params } = this.traceListWhere(projectId, filters);
    const rows = await this.query<{ bucket: string; c: unknown }>(
      `
      SELECT to_char(t."timestamp", ${fmt}) AS bucket, COUNT(*) AS c
      FROM traces t
      WHERE ${where}
      GROUP BY bucket
      ORDER BY bucket ASC
  ...
  async traceFacets(
    ...
    // composes with the unaliased facet queries (and the LATERAL unnest tags variant) unchanged.
    ...
    // One {value, count} list per facet, ordered by frequency. Tags is a text[] column,
    // so it's unnested with CROSS JOIN LATERAL before grouping (empty/NULL tag arrays
    // simply contribute no rows — unnest of an empty array yields nothing).
    ...
    const [environments, names, tags, scores, levels, types] = await Promise.all([
      ...
      facet(
        `SELECT tag AS value, COUNT(*) AS count FROM traces
         CROSS JOIN LATERAL unnest(tags) AS tv(tag)
  ...
  async listSessions(
    ...
    const rows = await this.query<SessionSummary>(
      ...
      SELECT
        ...
        to_char(MIN(t."timestamp"), ${ISO_FMT}) AS first_seen,
        to_char(MAX(t."timestamp"), ${ISO_FMT}) AS last_seen,
  ...
  async listObservationsByTrace(projectId: string, traceId: string): Promise<ObservationDetail[]> {
    const rows = await this.query<ObservationDetail>(
      ...
      SELECT
        ...
        to_char(start_time, ${ISO_FMT}) AS start_time,
        to_char(end_time, ${ISO_FMT}) AS end_time,
        level,
        ...
        cache_read_tokens, cache_creation_tokens, reasoning_tokens, total_cost, latency_ms,
        COALESCE(input, '') AS input,
        COALESCE(output, '') AS output,
        COALESCE(metadata, '{}') AS metadata
      FROM observations
  ...
  async evaluatorScoreTrend(projectId: string, days: number): Promise<EvalScoreTrendRow[]> {
    const rows = await this.query<{ date: string; name: string; cnt: unknown; avg_value: unknown }>(
      ...
      FROM scores
      WHERE project_id = ? AND source = 'EVAL' AND "timestamp" >= ? AND "value" IS NOT NULL
      GROUP BY to_char("timestamp", 'YYYY-MM-DD'), name
  ...
  async metricWindowSeries(projectId: string, windowMinutes: number, buckets: number): Promise<WindowMetric[]> {
    ...
    const obsBucket = "FLOOR(extract(epoch FROM (start_time - ?::timestamp)) / ?)";
    const traceBucket = 'FLOOR(extract(epoch FROM ("timestamp" - ?::timestamp)) / ?)';
    const [gens, traces] = await Promise.all([
      this.query<{
        b: unknown;
        generations: unknown;
  ...
  async widgetSeries(
    ...
    const AGG: Record<Exclude<WidgetMetric, "score">, string> = {
      ...
      generations: "COUNT(*)",
      latency_p95: "percentile_cont(0.95) WITHIN GROUP (ORDER BY o.latency_ms)",
      // The ::double precision cast is load-bearing: bigint / bigint is integer division in PG.
      error_rate: "(COUNT(*) FILTER (WHERE o.level = 'ERROR'))::double precision / COUNT(*)",
    };
    const GROUP: Record<WidgetBreakdown, string> = {
      by_day: "to_char(o.start_time, 'YYYY-MM-DD')",
      by_model: "o.model",
      by_user: "t.user_id",
      by_session: "t.session_id",
    };
  ...
  async runAnalyticsQuery(projectId: string, query: AnalyticsQuery): Promise<QueryResult> {
    ...
    // Metric columns (`${agg}_${measure}`) come back as strings from node-pg — coerce to numbers
    // at the store boundary; dimension/time columns stay strings.
  ...
  async getTraceRowsByIds(projectId: string, traceIds: string[]): Promise<TraceRow[]> {
    ...
    const rows = await this.query<TraceRow & { tags: unknown }>(
      ...
      SELECT
        project_id, id,
        to_char("timestamp", ${ISO_MS_FMT}) AS "timestamp",
  ...
  async getObservationRowsByIds(projectId: string, observationIds: string[]): Promise<ObservationRow[]> {
    ...
    const rows = await this.query<ObservationRow>(
      ...
      SELECT
        ...
        to_char(start_time, ${ISO_MS_FMT}) AS start_time,
        to_char(end_time, ${ISO_MS_FMT}) AS end_time,
        environment, level,
        COALESCE(status_message, '') AS status_message,
        model, provider,
  ...
  async listEmbeddingsForProjection(
    ...
    const rows = await this.query<{ observation_id: string; trace_id: string; vector: unknown }>(
      `
      SELECT observation_id, trace_id, vector::text AS vector
      FROM embeddings
  ...
  async getTraceEmbeddings(projectId: string, traceId: string): Promise<TraceEmbeddingRow[]> {
    const rows = await this.query<{
      ...
      `
      SELECT observation_id, trace_id, model, dim, vector::text AS vector
      FROM embeddings
  ...
  async rankSimilarTraceIds(
    ...
    // Each seed vector becomes a `vector <=> ?::vector` cosine-distance term; a candidate's
    // distance is the SMALLEST across seeds (its closest match). Exact scan, no ANN index
    // (dimensionless pgvector columns can't be indexed) — same recall contract as Doris.
    // The (model, dim) scope in WHERE is mandatory: `<=>` errors on dimension mismatch, and
    // the projection is only evaluated on rows that pass the WHERE.
    const params: unknown[] = [];
    const distTerms = seedVectors.map((v) => {
      params.push(toVectorLiteral(v));
    ...
    const shapes: { [K in TelemetryTable]: { select: string; map: (r: never) => TelemetryRowMap[K] } } = {
      ...
      observations: {
        select: `project_id, trace_id, id, type, parent_observation_id, name,
          ...
          reasoning_tokens, input_cost, output_cost, total_cost, prompt_id, prompt_version,
          COALESCE(input, '') AS input, COALESCE(output, '') AS output,
      ...
      scores: {
        select: `project_id, id, trace_id, observation_id, name,
          ...
          config_id, to_char(event_ts, ${ISO_MS_FMT}) AS event_ts`,
      ...
      embeddings: {
        select: `project_id, observation_id, trace_id, kind, model, dim,
          vector::text AS vector, to_char(event_ts, ${ISO_MS_FMT}) AS event_ts`,
