...
/**
 * Postgres dialect of the dashboard/widget analytics compiler (see doris/query.ts —
 * same view registry and validation; only the SQL surface differs): percentiles via
 * percentile_cont, time buckets via to_char(date_trunc(...)), double-quoted aliases.
 * All identifiers resolve against the registry or fixed enums; every value is bound.
 */
...
/** SQL for an aggregation over a measure. COUNT is always COUNT(*); percentiles are exact (cont). */
function aggSql(agg: string, measure: Measure): string {
  if (agg === "count") return "COUNT(*)";
  const col = measure.sql;
  if (agg === "uniq") return `COUNT(DISTINCT ${col})`;
  if (agg in PCT) return `percentile_cont(${PCT[agg]}) WITHIN GROUP (ORDER BY ${col})`;
...
/** Compile a validated AnalyticsQuery into parameterized Postgres SQL. */
export function compileQuery(projectId: string, query: AnalyticsQuery): { sql: string; params: unknown[] } {
  ...
  if (query.timeDimension) {
    ...
    const bucket = `to_char(date_trunc('${gran}', ${view.timeColumn}), 'YYYY-MM-DD"T"HH24:MI:SS')`;
