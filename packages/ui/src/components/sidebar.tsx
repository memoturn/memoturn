...
function SidebarProvider({
  ...
  // Internal state; use `open`/`onOpenChange` for a controlled sidebar. Seed from the persisted
  // cookie so the collapsed/expanded state survives a reload in SPAs (shadcn's default relies on a
  // server component to read the cookie into `defaultOpen`, which SPAs don't have).
  ...
  const setOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      // Persist the collapsed/expanded state across reloads.
  ...
  const contextValue = React.useMemo<SidebarContextProps>(
    ...
    // setOpenMobile is a stable useState setter — Biome flags it as an extra dep.
...
function Sidebar({
  ...
  return (
    <div
      ...
      data-slot="sidebar"
    >
      {/* Gap that reserves the sidebar's footprint in the flow. */}
      <div
...
function SidebarMenuSkeleton({
  ...
  // Random width between 50% and 90% keeps the skeleton from looking uniform.
