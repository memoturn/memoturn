...
app.use("/v1/retrieval/*", requireAuth);
...
// `format=items` is a generic backup dump; `format=oai-chat` and `format=anthropic-messages`
// emit provider-shaped fine-tuning lines (items without expectedOutput are skipped; count in
// X-Memoturn-Skipped). `version=N` exports a frozen version's items instead of the live set.
app.get("/v1/datasets/:name/export", async (c) => {
  const name = c.req.param("name");
  const q = new URL(c.req.url).searchParams;
  const requested = q.get("format");
  const format: DatasetExportFormat =
    requested === "oai-chat" ? "oai-chat" : requested === "anthropic-messages" ? "anthropic-messages" : "items";
...
// ── Retrieval diagnostics (RAG quality across traces) ────────────────────────────
app.openapi(
  createRoute({
    method: "get",
    path: "/v1/retrieval/analytics",
    summary: "Cross-trace retrieval diagnostics: score distribution, weakest retrievals, per-document stats",
    tags: ["embeddings"],
    security,
    request: {
      query: z.object({
    ...
    responses: {
      200: { description: "Retrieval analytics", content: { "application/json": { schema: C.retrievalAnalytics } } },
    },
  }),
  async (c) => {
    const { days, limit } = c.req.valid("query");
    return c.json(await getRetrievalAnalytics(c.get("projectId"), { days, limit }));
  },
);
