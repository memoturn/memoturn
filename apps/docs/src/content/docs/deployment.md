...
- For enterprise tenants, register an SSO provider (OIDC/SAML) per organization from the
  Organizations page, and configure the event sink (CDP forwarding) / PII masking per project as
  needed.
- Walk the [security hardening guide](/hardening/) — a checklist of every security knob
