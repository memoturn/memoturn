...
import { applyAllRetention } from "@memoturn/server";
 ...
 * Runs the ingest processor (+ online evaluations) and a daily retention cron.
 */
...
const maintenanceQueue = new Queue(QUEUE_NAMES.export, { connection: connectionOptions(), prefix: QUEUE_PREFIX });
...
// Schedule retention daily (idempotent — same jobId/pattern just updates the schedule).
await maintenanceQueue.add(
  "retention",
  {},
  { repeat: { pattern: "0 3 * * *" }, jobId: "retention-daily", removeOnComplete: true },
);
...
console.log("[worker] memoturn worker started (ingest + retention cron)");
