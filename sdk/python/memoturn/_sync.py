"""Synchronous client. Every method is a thin send of an `_ops` request
spec; `_async.py` mirrors this file exactly with awaits."""

from __future__ import annotations

from typing import Any, Iterator, List, Optional

import httpx

from . import _ops
from ._errors import MemoturnError
from ._ops import Op
from ._types import (
    AskResult,
    ErasureCoupon,
    IngestResult,
    Memory,
    RecallResult,
    Turn,
    TurnAppendResult,
)


class _Wire:
    def __init__(
        self,
        url: str,
        token: Optional[str],
        platform_key: Optional[str],
        client: Optional[httpx.Client],
    ):
        self.base = url.rstrip("/")
        self.token = token
        self.platform_key = platform_key
        self._owns_client = client is None
        self.http = client or httpx.Client(timeout=30.0)

    def send(self, op: Op) -> Any:
        cred = (
            (self.platform_key or self.token) if op.platform else (self.token or self.platform_key)
        )
        headers = {"authorization": f"Bearer {cred}"} if cred else {}
        resp = self.http.request(
            op.method, f"{self.base}{op.path}", json=op.json, content=op.content, headers=headers
        )
        if resp.is_error:
            if resp.status_code == 404 and op.none_on_404:
                return None
            try:
                body = resp.json()
                message, code = body.get("error", resp.text), body.get("code")
            except Exception:
                message, code = resp.text, None
            raise MemoturnError(resp.status_code, message, code)
        return op.parse(resp)

    def close(self) -> None:
        if self._owns_client:
            self.http.close()


class TranscriptSession:
    """Append-only verbatim transcript (``__memoturn_messages``) for one session."""

    def __init__(self, wire: _Wire, db: str, session_id: str):
        self._w = wire
        self._db = db
        self.session_id = session_id

    def append_turn(
        self, role: str, content: Any, embedding: Optional[List[float]] = None
    ) -> TurnAppendResult:
        return self._w.send(_ops.turn_append(self._db, self.session_id, role, content, embedding))

    def get_window(self, last: int = 20) -> List[Turn]:
        return self._w.send(_ops.turn_window(self._db, self.session_id, last))

    def search_semantic(self, vector: List[float], k: int = 5) -> List[Turn]:
        return self._w.send(_ops.turn_search(self._db, self.session_id, vector, k))


class MemoryProfile:
    """One memory profile: the isolated store every agent serving this
    user/team/persona shares. Backed by its own database (``{ns}--{profile}``),
    so ``checkpoint``/``rewind``/``fork`` operate on the whole memory atomically.
    """

    def __init__(
        self,
        wire: _Wire,
        namespace: str,
        profile: str,
        branch: Optional[str] = None,
        source: Optional[str] = None,
    ):
        self._w = wire
        self.namespace = namespace
        self.profile = profile
        self._branch = branch
        self._source = source

    @property
    def _db(self) -> str:
        return f"{self.namespace}--{self.profile}"

    def on_branch(self, branch: str) -> "MemoryProfile":
        """Address a branch of this profile's memory (burner experiments)."""
        return MemoryProfile(self._w, self.namespace, self.profile, branch, self._source)

    def ingest(self, memories: List[dict]) -> IngestResult:
        """Idempotent batch ingest; the profile auto-creates on first call.

        Each memory: ``{type, topic_key?, summary, content, keywords?,
        embedding?, session_id?, source?, ttl?}``. ``source`` (originating
        agent, e.g. "claude-code") defaults to the client-level ``source``.
        """
        memories = _ops.with_default_source(memories, self._source)
        return self._w.send(
            _ops.memories_ingest(self.namespace, self.profile, self._branch, memories)
        )

    def recall(
        self,
        query: Optional[str] = None,
        *,
        embedding: Optional[List[float]] = None,
        topic_key: Optional[str] = None,
        types: Optional[List[str]] = None,
        session_id: Optional[str] = None,
        source: Optional[str] = None,
        k: int = 8,
        include_superseded: bool = False,
        include_turns: bool = False,
    ) -> RecallResult:
        """Hybrid recall; empty ``memories`` means nothing relevant (never pads).

        ``source`` filters to memories ingested by one agent (e.g. "claude-code").
        """
        body = _ops.recall_body(
            query, embedding, topic_key, types, session_id, source, k,
            include_superseded, include_turns,
        )
        return self._w.send(_ops.memories_recall(self.namespace, self.profile, self._branch, body))

    def ask(
        self,
        question: str,
        *,
        types: Optional[List[str]] = None,
        session_id: Optional[str] = None,
        source: Optional[str] = None,
        k: int = 8,
        include_superseded: bool = False,
    ) -> AskResult:
        """Ask a natural-language question over this profile's memories:
        hybrid recall, then the node's assistant synthesizes a prose answer
        citing the supporting memory ids (opt-in node feature). Returns
        ``{answer, sources, memories, txid}``; ``answer`` is None when nothing
        relevant was recalled. Raises MemoturnError (code ``unconfigured``)
        when the node has no assistant — fall back to ``recall()`` and
        synthesize yourself.
        """
        body = _ops.ask_body(question, types, session_id, source, k, include_superseded)
        return self._w.send(_ops.memories_ask(self.namespace, self.profile, self._branch, body))

    def extract(
        self,
        turns: List[dict],
        *,
        session_id: Optional[str] = None,
        source: Optional[str] = None,
        dry_run: bool = False,
    ) -> dict:
        """Server-side extraction (opt-in node feature): distill raw turns into
        typed memories with a control-plane LLM, then ingest. ``dry_run``
        returns proposals without writing. Raises MemoturnError (code
        ``unconfigured``) when the node has no extractor. ``source`` defaults
        to the client-level ``source``.
        """
        body = _ops.extract_body(
            turns, session_id, source if source is not None else self._source, dry_run
        )
        return self._w.send(_ops.memories_extract(self.namespace, self.profile, self._branch, body))

    def get(self, memory_id: str) -> Optional[Memory]:
        """One memory with its supersession chain, or None."""
        return self._w.send(_ops.memory_get(self.namespace, self.profile, self._branch, memory_id))

    def forget(self, memory_id: str) -> bool:
        """Hard delete (supersession already preserves history without this)."""
        found = self._w.send(
            _ops.memory_forget(self.namespace, self.profile, self._branch, memory_id)
        )
        return bool(found)

    def erase(
        self,
        *,
        memory_id: Optional[str] = None,
        topic_key: Optional[str] = None,
        type: Optional[str] = None,
        session_id: Optional[str] = None,
        turns: bool = False,
    ) -> ErasureCoupon:
        """Verifiable erasure (ADR-0010): hard-forget now with secure_delete,
        then a bounded-time history rewrite proven by a signed receipt.
        Target exactly one of ``memory_id``, ``topic_key`` (with ``type``),
        or ``session_id``. Poll :meth:`erasure` for the receipt."""
        body = _ops.erase_body(memory_id, topic_key, type, session_id, turns)
        return self._w.send(_ops.erase(self.namespace, self.profile, self._branch, body))

    def erasures(self) -> List[ErasureCoupon]:
        """Erasure coupons for this profile, newest first."""
        return self._w.send(_ops.erasures_list(self.namespace, self.profile, self._branch))

    def erasure(self, erasure_id: str) -> Optional[ErasureCoupon]:
        """One erasure coupon — ``completed`` carries the signed receipt."""
        return self._w.send(
            _ops.erasure_get(self.namespace, self.profile, self._branch, erasure_id)
        )

    def sessions(self) -> List[dict]:
        return self._w.send(_ops.sessions_list(self.namespace, self.profile, self._branch))

    def end_session(self, session_id: str, *, turns: bool = False) -> None:
        """End a session: its task memories go; ``turns=True`` drops the transcript."""
        self._w.send(_ops.session_end(self.namespace, self.profile, self._branch, session_id, turns))

    def checkpoint(self, name: str) -> dict:
        """Checkpoint the whole memory (requires admin scope)."""
        return self._w.send(_ops.checkpoint(self._db, self._branch or "main", name))

    def rewind(self, to: Any) -> None:
        """Rewind the whole memory to a checkpoint or txid (admin scope)."""
        self._w.send(_ops.rewind(self._db, self._branch or "main", to))

    def fork(
        self, branch: str, *, from_branch: Optional[str] = None, ttl: Optional[int] = None
    ) -> "MemoryProfile":
        """Fork the memory copy-on-write; ``ttl`` makes it a burner branch."""
        self._w.send(_ops.branch_create(self._db, branch, from_branch or self._branch, ttl))
        return self.on_branch(branch)

    def session(self, session_id: str) -> TranscriptSession:
        """The raw conversation transcript layer for one session."""
        return TranscriptSession(self._w, self._db, session_id)


class _Kv:
    def __init__(self, wire: _Wire, spec: str):
        self._w = wire
        self._spec = spec

    def put(self, ns: str, key: str, value: str, *, ttl: Optional[int] = None) -> int:
        return self._w.send(_ops.kv_put(self._spec, ns, key, value, ttl))

    def get(self, ns: str, key: str) -> Optional[str]:
        return self._w.send(_ops.kv_get(self._spec, ns, key))

    def delete(self, ns: str, key: str) -> None:
        self._w.send(_ops.kv_delete(self._spec, ns, key))

    def list(self, ns: str, *, prefix: str = "", limit: int = 100) -> List[str]:
        return self._w.send(_ops.kv_list(self._spec, ns, prefix, limit))


class Collection:
    def __init__(self, wire: _Wire, spec: str, name: str):
        self._w = wire
        self._spec = spec
        self.name = name

    def insert(self, docs: List[dict]) -> dict:
        return self._w.send(_ops.docs_insert(self._spec, self.name, docs))

    def find(
        self,
        filter: Optional[dict] = None,
        *,
        sort: Optional[dict] = None,
        limit: Optional[int] = None,
        skip: Optional[int] = None,
    ) -> List[dict]:
        return self._w.send(_ops.docs_find(self._spec, self.name, filter, sort, limit, skip))

    def update(self, filter: dict, update: dict, *, multi: bool = False) -> dict:
        return self._w.send(_ops.docs_update(self._spec, self.name, filter, update, multi))

    def delete(self, filter: dict, *, multi: bool = False) -> dict:
        return self._w.send(_ops.docs_delete(self._spec, self.name, filter, multi))

    def create_index(self, path: str) -> None:
        self._w.send(_ops.docs_create_index(self._spec, self.name, path))


class _Vectors:
    def __init__(self, wire: _Wire, spec: str):
        self._w = wire
        self._spec = spec

    def upsert(self, collection: str, id: str, embedding: List[float]) -> int:
        return self._w.send(_ops.vectors_upsert(self._spec, collection, id, embedding))

    def search(self, collection: str, vector: List[float], *, k: int = 10) -> List[dict]:
        return self._w.send(_ops.vectors_search(self._spec, collection, vector, k))


class _Branch:
    def __init__(self, wire: _Wire, spec: str):
        self._w = wire
        self._spec = spec

    def create(
        self, name: str, *, from_branch: Optional[str] = None, ttl: Optional[int] = None
    ) -> dict:
        return self._w.send(_ops.branch_create(self._spec, name, from_branch, ttl))

    def list(self) -> List[dict]:
        return self._w.send(_ops.branch_list(self._spec))

    def delete(self, name: str) -> None:
        self._w.send(_ops.branch_delete(self._spec, name))

    def checkpoint(self, branch: str, name: str) -> dict:
        return self._w.send(_ops.checkpoint(self._spec, branch, name))

    def rewind(self, branch: str, to: Any) -> None:
        self._w.send(_ops.rewind(self._spec, branch, to))


class Db:
    """One database (``name`` or ``name@branch``) — the multi-model substrate."""

    def __init__(self, wire: _Wire, spec: str):
        self._w = wire
        self.spec = spec
        self.kv = _Kv(wire, spec)
        self.vectors = _Vectors(wire, spec)
        self.branch = _Branch(wire, spec)

    def sql(self, q: str, params: Optional[list] = None) -> dict:
        """SQL escape hatch (atomic batch of one statement)."""
        return self._w.send(_ops.sql(self.spec, q, params))

    def collection(self, name: str) -> Collection:
        return Collection(self._w, self.spec, name)

    def sync(self) -> int:
        """Ship this branch's state to object storage now (durability point)."""
        return self._w.send(_ops.sync_db(self.spec))


class Memoturn:
    """Synchronous client. Usable as a context manager::

        with Memoturn(url, token=token) as mt:
            mt.memory("acme", "alice").recall(query="…")
    """

    def __init__(
        self,
        url: str = "http://127.0.0.1:8080",
        *,
        token: Optional[str] = None,
        platform_key: Optional[str] = None,
        source: Optional[str] = None,
        http_client: Optional[httpx.Client] = None,
    ):
        self._w = _Wire(url, token, platform_key, http_client)
        self._source = source

    def __enter__(self) -> "Memoturn":
        return self

    def __exit__(self, *exc: Any) -> None:
        self.close()

    def close(self) -> None:
        """Close the underlying HTTP client (only if this client owns it)."""
        self._w.close()

    def memory(self, namespace: str, profile: str) -> MemoryProfile:
        """The memory surface: one profile per user/team/agent persona."""
        return MemoryProfile(self._w, namespace, profile, source=self._source)

    def profiles(self, namespace: str) -> List[dict]:
        """Profiles under a namespace (requires a namespace token)."""
        return self._w.send(_ops.profiles_list(namespace))

    def db(self, spec: str) -> Db:
        """A database / branch on the multi-model substrate."""
        return Db(self._w, spec)

    # ---- control plane (platform key) ----

    def create_database(self, name: str) -> dict:
        return self._w.send(_ops.db_create(name))

    def list_databases(self) -> List[dict]:
        return self._w.send(_ops.db_list())

    def delete_database(self, name: str) -> None:
        self._w.send(_ops.db_delete(name))

    def create_token(self, db: str, scope: str, *, expires_in: Optional[int] = None) -> str:
        """Mint a per-database token (platform key)."""
        return self._w.send(_ops.token_create(db, scope, expires_in))

    def create_namespace_token(
        self, namespace: str, scope: str, *, expires_in: Optional[int] = None
    ) -> str:
        """Mint a namespace token covering every profile under it (platform key)."""
        return self._w.send(_ops.ns_token_create(namespace, scope, expires_in))

    # ---- data governance (ADR-0010) ----

    def get_policy(self, namespace: str, *, profile: Optional[str] = None) -> Optional[dict]:
        """The namespace policy document (platform key; ``None`` when unset),
        or — with ``profile`` — the override plus the effective policy
        actually enforced (read token)."""
        if profile is not None:
            return self._w.send(_ops.profile_policy_get(namespace, profile))
        return self._w.send(_ops.ns_policy_get(namespace))

    def set_policy(
        self, namespace: str, policy: Optional[dict], *, profile: Optional[str] = None
    ) -> dict:
        """Set the namespace policy (platform key), or — with ``profile`` —
        a tighten-only profile override (admin token; loosening any field is
        a 409). ``policy=None`` with ``profile`` clears the override."""
        if profile is not None:
            return self._w.send(_ops.profile_policy_set(namespace, profile, policy))
        return self._w.send(_ops.ns_policy_set(namespace, policy))

    def audit_events(
        self,
        namespace: str,
        *,
        from_ms: Optional[int] = None,
        to_ms: Optional[int] = None,
        action: Optional[str] = None,
        profile: Optional[str] = None,
        outcome: Optional[str] = None,
        limit: int = 100,
    ) -> Iterator[dict]:
        """Iterate a namespace's audit stream, oldest first, paginating
        transparently (requires ``audit.enabled`` in its policy; platform key
        or namespace admin token). Events are metadata only — never memory
        content."""
        cursor: Optional[str] = None
        while True:
            body = self._w.send(
                _ops.audit_page(
                    namespace,
                    {
                        "from": from_ms,
                        "to": to_ms,
                        "action": action,
                        "profile": profile,
                        "outcome": outcome,
                        "limit": limit,
                        "cursor": cursor,
                    },
                )
            )
            yield from body["events"]
            cursor = body.get("next_cursor")
            if body.get("complete") or not cursor:
                return
